...
def get_test_query_embedding(args):
    ...
    test_dataset = ConvDataset(args, query_tokenizer, None, args.test_file_path, add_doc_info=False)
    test_loader = DataLoader(test_dataset, 
                            batch_size = args.eval_batch_size, 
                            shuffle=False, 
                            collate_fn=test_dataset.get_collate_fn(args, add_doc_info = False, mode = "test_cast"))
...
def output_test_res(query_embedding2id,
    ...
    # all passages
    if args.need_output_passage_content:
        all_passages = load_collection(args.passage_collection_path)
    
    # write to file
    logger.info('begin to write the output...')

    output_file = oj(args.qrel_output_path, 'res.json')
    output_trec_file = oj(args.qrel_output_path, 'res.trec')
    ...
    # print result
    res = print_res(output_trec_file, args.gold_qrel_file_path, args.rel_threshold)
    return res
...
if __name__ == '__main__':
    main()
    # args = get_args()
    # print_res("../ConvDR/results/baseline_cast19_quretec/res.trec", "../Dataset/cast19/preprocessed/qrels.tsv", 1)
